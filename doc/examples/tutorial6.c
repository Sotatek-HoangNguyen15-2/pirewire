...
static void client_info(void *object, const struct pw_client_info *info)
{
	...
	printf("client: id:%u, \tchange mask: %ld\n", info->id,info->change_mask);
	printf("\tprops:\n");
...
/* [client_permission] */
static void client_permission (void *data, uint32_t index, uint32_t n_permissions, const struct pw_permission *permissions)
{
	struct data *p_data=data;
	// const struct spa_dict_item *item;

	// printf("client: id:%u\n", info->id);
	printf("\tid: %d, \tpermissions: %d\n",permissions->id,permissions->permissions);
	// spa_dict_for_each(item, info->props)
	// 	printf("\t\t%s: \"%s\"\n", item->key, item->value);

	pw_main_loop_quit(p_data->loop);
}
			...
			uint32_t version, const struct spa_dict *props)
{
	...
	printf("global: id:%u, \tpermissions: %u\n", id,permissions);
	printf("\tprops:\n");
